...
func TestLoad_DefaultPanels(t *testing.T) {
	...
	if len(cfg.Panels.Visible) != len(expectedPanels) {
		t.Errorf("Panels.Visible has %d items, want %d", len(cfg.Panels.Visible), len(expectedPanels))
	} else {
...
func TestLoad_NamespaceFallback(t *testing.T) {
	...
	if cfg.Namespace != cfg.Defaults.Namespace {
		t.Errorf("Namespace = %q, want %q (from Defaults.Namespace)", cfg.Namespace, cfg.Defaults.Namespace)
	}
}
